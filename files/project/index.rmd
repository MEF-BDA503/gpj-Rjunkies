...
## What's the progress of survival rate?

We observe that Survival rate start from approximately 0% in the early 1900's to 45%.  Due to war and no safety concern in in 1910s, survival rate almost 0. As seen in graph, at the World War II, survival rate decrease to 5%.
...
ggplot(dataSurvival, aes(y=SurvivalRate, x=Year)) +  
  geom_col(aes(y=TotalAccident/n)) +
  geom_line(aes(group = 1), size=1.3, alpha=1) +
...
## Possible Cause of Crash - Word Cloud

For each accident, there is an explanation of the accident. By looking this summary information, we try to visuailize of summary information as a word cloud.
